...
async def give_filter(client, message):
    # ── AUTH_GROUPS check ────────────────────────────────────────────────────
    # Agar AUTH_GROUPS set hai toh sirf usi/unhi groups mein kaam karega
     ...
     #   if not search:
      ...
      #      return
        ...
        if query.from_user.id in SUDO_USERS:
            buttons.append([InlineKeyboardButton('⚙️ ᴀᴅᴍɪɴ ᴏɴʟʏ 🔧', callback_data='admin')])
        buttons += [[ 
        ...
        if query.from_user.id not in SUDO_USERS:
            return await query.answer("⚠️ Ye sirf bot admins ke liye hai.", show_alert=True)
        buttons = [[
            InlineKeyboardButton('Nᴇxᴛ ➡️', callback_data='admin2')
        ],[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help'),
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⟸ Hᴇʟᴘ Mᴇɴᴜ', callback_data='help'),
            InlineKeyboardButton('ᴇxᴛʀᴀ', callback_data='extra')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.EXTRAMOD_TXT.format(CHNL_LNK),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="https://t.me/Dmconnectorbot")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        ...
        for degs in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(degs[0]):
                await auto_filter(client, degs, msg, reply_msg, vj_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
